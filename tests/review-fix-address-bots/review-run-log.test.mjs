...
test("writes dated JSONL and derives reviewer overlap without inventing token usage", () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ["run_started", "reviewer_pass_completed", "run_finished"],
    );
...
test("sanitizes remote credentials and transport details", () => {
  ...
  assert.equal(
    sanitizeRemote("git@github.com:biw/skills.git", "fallback"),
  ...
  const derived = deriveMetrics({
    reviewers: [
      ...
      },
      {
        ...
        continuityChecks: [{ round: 1, verified: true, tokenUsage: null }],
        rounds: [
          { phase: "initial", round: 1, findingIds: ["F1", "F3"], tokenUsage: { totalTokens: 80 } },
        ],
      },
      {
        reviewerId: "terra-2",
      ...
      },
      {
        ...
        continuityChecks: [{ round: 1, verified: true, tokenUsage: null }],
        rounds: [
          { phase: "initial", round: 1, findingIds: ["F1", "F5"], tokenUsage: { totalTokens: 40 } },
        ],
      },
      {
        reviewerId: "luna-2",
