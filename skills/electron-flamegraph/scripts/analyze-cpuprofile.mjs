...
if (!profilePath) {
  console.error(
    "usage: analyze-cpuprofile.mjs <path-to-cpuprofile> [--top=N] [--sampling-gap-threshold-ms=N] [--json]",
  );
  exit(1);
}
...
const samplingGapMs = flagVal("--sampling-gap-threshold-ms", flagVal("--stall-threshold-ms", 50));
...
if (!profile.nodes || !profile.samples || !profile.timeDeltas) {
  console.error(
    `${profilePath} does not look like a V8 cpuprofile (missing nodes/samples/timeDeltas)`,
  );
  exit(1);
}
...
const isNodeModule = (frame) =>
  typeof frame.url === "string" && frame.url.includes("/node_modules/");
...
if (syncFsUs > totalSampledUs * 0.02) {
  addWarning(
    "high",
    `Synchronous fs calls account for ${(syncFsUs / 1000).toFixed(0)}ms — move to async equivalents, especially on the hot path.`,
  );
}

// JSON
const jsonParseUs = sumWhere((f) => /^JSON\.parse$/.test(f.functionName || ""));
const jsonStringifyUs =
  sumWhere((f) => f.functionName === "stringify" && /json/i.test(f.url || "")) +
  sumWhere((f) => /^JSON\.stringify$/.test(f.functionName || ""));
if (jsonParseUs > totalSampledUs * 0.05) {
  addWarning(
    "medium",
    `JSON.parse ~${(jsonParseUs / 1000).toFixed(0)}ms (${((jsonParseUs / totalSampledUs) * 100).toFixed(1)}%). Consider smaller payloads, streaming parser, or binary format.`,
  );
}
if (jsonStringifyUs > totalSampledUs * 0.05) {
  addWarning(
    "medium",
    `JSON.stringify ~${(jsonStringifyUs / 1000).toFixed(0)}ms (${((jsonStringifyUs / totalSampledUs) * 100).toFixed(1)}%). Likely serializing too-large objects across IPC or to disk.`,
  );
}
...
if (moduleLoadUs > totalSampledUs * 0.15) {
  addWarning(
    "medium",
    `Module loading ~${(moduleLoadUs / 1000).toFixed(0)}ms (${((moduleLoadUs / totalSampledUs) * 100).toFixed(1)}%). If this isn't a startup profile, something is require()-ing on the hot path — lazy-load or precompile.`,
  );
}
...
if (gcPct > 5) {
  addWarning(
    "medium",
    `Garbage collector took ${gcPct.toFixed(1)}% of sampled time. Allocation-heavy hot path — look for per-call object/array/string creation in loops.`,
  );
}
...
if (cryptoUs > totalSampledUs * 0.1) {
  addWarning(
    "medium",
    `crypto/zlib ~${(cryptoUs / 1000).toFixed(0)}ms (${((cryptoUs / totalSampledUs) * 100).toFixed(1)}%). Candidate for offloading to a worker thread.`,
  );
}
    ...
    JSON.stringify(
      {
        ...
        categories: Object.fromEntries(
          Object.entries(categorySelfUs).map(([k, v]) => [
            k,
            { us: v, pct: totalSampledUs ? (v / totalSampledUs) * 100 : 0 },
          ]),
        ),
        topSelf,
        topTotal,
...
console.log(
  `Sample interval: ~${samples.length > 0 ? (totalSampledUs / samples.length).toFixed(0) : "?"}μs avg`,
);
...
if (samplingGaps.length === 0) {
  console.log(
    `  None detected. This does not prove the event loop remained responsive; CPU profiles do not directly measure event-loop delay.`,
  );
} else {
  console.log(
    `  ${samplingGaps.length} gap(s). Showing top ${Math.min(10, samplingGaps.length)}. A gap is not proof of a main-thread stall.\n`,
  );
  for (const gap of samplingGaps.slice(0, 10)) {
    const stack = stackFor(gap.nodeId);
    console.log(
      `  ${fmtMs(gap.durationUs)} sampling gap — sample after gap: ${stack[0] ? labelNode(stack[0]) : "(unknown)"}`,
    );
