...
3. Give every configured reviewer the same self-contained raw review prompt, review target, and reviewer role boundary. For an initial native `spawn_agent` launch, set `fork_turns` to `"none"`; never inherit the primary agent's full conversation. A full-history fork must inherit the parent's model and would both prevent the independent model cohort and add irrelevant context. Tell reviewers to inspect the repository themselves and report every qualifying finding with file, minimal line range, severity, scenario, and concise rationale.
