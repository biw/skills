...
3. Update the frontmatter so `name` exactly matches the folder name and `description` explains what the skill does and when agents should use it.
4. Add optional `scripts/`, `references/`, or `assets/` only when they directly support the skill.
5. Run `pnpm validate` when the repo provides that script.
6. Run `pnpm skills add . --list` to confirm the CLI can discover the skill locally.
